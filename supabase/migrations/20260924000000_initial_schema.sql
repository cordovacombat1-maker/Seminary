-- Seminary app: initial schema
-- Run this once in Supabase: Dashboard -> SQL Editor -> New query -> paste -> Run.
-- Row Level Security is ON for every table. Students can read only their own rows.
-- All grading, progress and chat writes happen in Netlify Functions using the
-- service-role key (which bypasses RLS), so students cannot forge scores.
...
-- ---------------------------------------------------------------------------
-- Reference data (loaded once by the scripts in /scripts)
-- ---------------------------------------------------------------------------
...
create index library_chunks_embedding on public.library_chunks
...
as $$
  select c.id, c.work_id, c.author, c.title, c.tradition, c.section_ref, c.source_url, c.content,
         ts_rank(c.content_tsv, websearch_to_tsquery('english', query_text))::double precision as similarity
  from public.library_chunks c
...
alter table public.morphology_codes enable row level security;
alter table public.proper_names enable row level security;
...
create policy "own enrollments" on public.enrollments for select to authenticated using (user_id = (select auth.uid()));
create policy "own lesson progress" on public.lesson_progress for select to authenticated using (user_id = (select auth.uid()));
create policy "own objectives" on public.objective_progress for select to authenticated using (user_id = (select auth.uid()));
create policy "own chat" on public.chat_messages for select to authenticated using (user_id = (select auth.uid()));
create policy "own submitted quizzes" on public.quiz_attempts for select to authenticated
  using (user_id = (select auth.uid()) and submitted_at is not null);
create policy "own papers" on public.papers for select to authenticated using (user_id = (select auth.uid()));
create policy "own certificates" on public.certificates for select to authenticated using (user_id = (select auth.uid()));
create policy "own flags" on public.flags for select to authenticated using (user_id = (select auth.uid()));
create policy "create own flags" on public.flags for insert to authenticated with check (user_id = (select auth.uid()));

-- Reference data: any signed-in student may read
create policy "read verses" on public.bible_verses for select to authenticated using (true);
create policy "read words" on public.original_words for select to authenticated using (true);
create policy "read lexicon" on public.lexicon for select to authenticated using (true);
create policy "read morphology" on public.morphology_codes for select to authenticated using (true);
create policy "read names" on public.proper_names for select to authenticated using (true);
create policy "read library" on public.library_chunks for select to authenticated using (true);

-- Students may only change their display name, nothing else on their profile
revoke update on public.profiles from authenticated;
grant update (full_name) on public.profiles to authenticated;
-- Students may only set these columns when creating a flag
revoke insert on public.flags from authenticated;
grant insert (user_id, lesson_id, message_id, message_excerpt, reason) on public.flags to authenticated;
-- Nothing is visible to logged-out visitors
